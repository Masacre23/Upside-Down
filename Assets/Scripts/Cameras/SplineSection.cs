...
public class SplineSection : MonoBehaviour {
    ...
    {
        Quaternion qret = Quaternion.identity;
        if ((t < 0f) && (prev != null) && prev.IsValid())
        {
            return prev.GetRotationAt(t + 1.0f);
        }
        else if ((t > 1f) && (next != null) && next.IsValid())
        {
            return next.GetRotationAt(t - 1.0f);
        }
        else if (IsValid())
        {
            float t2 = t * t;
            float t3 = t2 * t;
            Vector3 p0 = startPoint.rotation.eulerAngles, p1 = endPoint.rotation.eulerAngles, m0 = startTangent.rotation.eulerAngles - p0, m1 = endTangent.rotation.eulerAngles - p1;
            Vector3 ret = (p0 * ((2.0f * t3) - (3.0f * t2) + 1.0f))
                + (m0 * (t3 + (-2.0f * t2) + t))
                + (p1 * ((-2.0f * t3) + (3.0f * t2)))
                + (m1 * (t3 - t2));
            qret.eulerAngles = ret;
            return qret;
        }
        else
        {
